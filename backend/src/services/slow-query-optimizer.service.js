...
exports.slowQueryOptimizerService = void 0;
...
class SlowQueryOptimizerService {
    ...
    async initialize() {
        if (!this.config.enabled) {
            logger_1.logger.info('Slow query optimization disabled');
            return;
        }
        try {
            logger_1.logger.info('Initializing slow query optimizer service...', {
            ...
            logger_1.logger.info('Slow query optimizer service initialized successfully');
        }
        catch (error) {
            logger_1.logger.error('Failed to initialize slow query optimizer service', { error });
            throw error;
        }
    }
    async configureMySQLSlowQueryLog() {
        try {
            // Enable slow query logging
            await connection_1.connection.execute('SET GLOBAL slow_query_log = ?', ['ON']);
            // Set threshold
            await connection_1.connection.execute('SET GLOBAL long_query_time = ?', [this.config.logThreshold]);
            // Log queries not using indexes
            await connection_1.connection.execute('SET GLOBAL log_queries_not_using_indexes = ?', ['ON']);
            // Log admin statements
            await connection_1.connection.execute('SET GLOBAL log_slow_admin_statements = ?', ['ON']);
            // Set minimum examined row threshold
            await connection_1.connection.execute('SET GLOBAL min_examined_row_limit = ?', [100]);
            logger_1.logger.info('MySQL slow query logging configured', {
                threshold: this.config.logThreshold,
                logQueriesNotUsingIndexes: true
            });
        }
        catch (error) {
            logger_1.logger.warn('Could not configure MySQL slow query logging - insufficient privileges', { error });
        }
    }
    ...
    async loadExistingSlowQueries() {
      ...
      `, [this.config.logThreshold]);
            ...
            logger_1.logger.info('Loaded existing slow queries from performance schema', {
                queriesLoaded: this.queryStats.size
            });
        }
        catch (error) {
            logger_1.logger.debug('Could not load from performance schema', { error: error.message });
        }
    }
    async enablePerformanceSchema() {
        try {
      ...
      `);
            logger_1.logger.info('Performance Schema enabled for statement monitoring');
        }
        catch (error) {
            logger_1.logger.debug('Could not configure Performance Schema', { error: error.message });
        }
    }
    setupScheduledTasks() {
        ...
        const analyzeTask = cron.schedule('0 * * * *', async () => {
            try {
                ...
                logger_1.logger.error('Scheduled slow query analysis failed', { error });
            }
        ...
        const optimizeTask = cron.schedule('0 2 * * *', async () => {
            try {
                ...
                logger_1.logger.error('Scheduled optimization generation failed', { error });
            }
        ...
        const cleanupTask = cron.schedule('0 1 * * *', async () => {
            try {
                ...
                logger_1.logger.error('Scheduled log cleanup failed', { error });
            }
        }, { name: 'log-cleanup' });
        ...
        logger_1.logger.info('Slow query optimizer scheduled tasks configured');
    }
    async analyzeSlowQueries() {
        if (!this.config.analyzeEnabled)
            return;
        try {
            logger_1.logger.info('Starting slow query analysis...');
            ...
            logger_1.logger.error('Slow query analysis failed', { error });
        }
    }
    async getRecentSlowQueries() {
      ...
      `, [this.config.logThreshold]);
            ...
            logger_1.logger.debug('Could not get recent slow queries from performance schema', { error });
            return [];
        }
    }
    ...
    async generateOptimizationRecommendations() {
        if (!this.config.optimizeEnabled)
            return;
        try {
            logger_1.logger.info('Generating optimization recommendations...');
            const problematicQueries = this.identifyProblematicQueries();
            ...
            logger_1.logger.info('Optimization recommendations generated', {
    ...
    async analyzeAndOptimizeQuery(stats) {
        try {
            // Get execution plan
            ...
            logger_1.logger.debug('Could not analyze query for optimization', {
    ...
    async getQueryExecutionPlan(query) {
        try {
            const [rows] = await connection_1.connection.execute(`EXPLAIN FORMAT=JSON ${query}`);
            return rows;
        }
        catch (error) {
            // Try simple EXPLAIN if JSON format fails
            try {
                const [rows] = await connection_1.connection.execute(`EXPLAIN ${query}`);
                return rows;
            }
            catch (fallbackError) {
                return [];
    ...
    generateQueryOptimization(stats, executionPlan) {
        const suggestions = [];
    ...
    estimateImprovement(stats, suggestionCount) {
    ...
    async generateIndexRecommendations() {
      ...
      `);
            ...
            logger_1.logger.error('Failed to generate index recommendations', { error });
        }
    }
    async analyzeTableIndexNeeds(tableName) {
        try {
            // Get existing indexes
            const [indexes] = await connection_1.connection.execute(`
      ...
      `);
            ...
            logger_1.logger.debug('Could not analyze index needs for table', { tableName, error });
        }
        return null;
    }
    extractCommonFilterColumns(queries, tableName) {
    ...
    async cleanupOldLogs() {
        try {
            ...
            if (removedCount > 0) {
                logger_1.logger.info('Old slow query statistics cleaned up', {
                    removedCount,
    ...
    async runQueryOptimizationReport() {
    ...
    async shutdown() {
        logger_1.logger.info('Shutting down slow query optimizer service...');
        this.scheduledTasks.forEach((task, name) => {
            task.stop();
            logger_1.logger.debug(`Stopped scheduled task: ${name}`);
        });
        this.scheduledTasks.clear();
        logger_1.logger.info('Slow query optimizer service shutdown completed');
    }
}
// Create and export singleton instance
exports.slowQueryOptimizerService = new SlowQueryOptimizerService();
exports.default = exports.slowQueryOptimizerService;
