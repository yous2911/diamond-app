...
export class SmartRateLimitService {
  ...
  /**
   * Get rate limit rule for a given key
   */
  public getRule(ruleKey: string): RateLimitRule | undefined {
...
export function createRateLimitMiddleware(ruleKey: string = 'api:general') {
  ...
  return async (request: any, reply: any) => {
    ...
    if (!result.allowed) {
      ...
      reply.headers({
        'X-RateLimit-Limit': rateLimitService.getRule(ruleKey)?.maxRequests || 0,
