...
exports.exerciseGeneratorService = exports.ExerciseGeneratorService = void 0;
// Map our difficulty levels to schema difficulty levels
const DIFFICULTE_MAP = {
    'decouverte': 'FACILE',
    'entrainement': 'MOYEN',
    'maitrise': 'MOYEN',
    'expert': 'DIFFICILE'
...
class ExerciseGeneratorService {
    ...
    generateExercisesBatch(niveau, matiere, difficulte = 'entrainement', count = 5, concepts) {
        const relevantTemplates = Array.from(this.templates.values()).filter(template => template.niveau === niveau &&
    ...
    generateMultiplication(minTable, maxTable) {
    ...
    calculateDuration(type, difficulte) {
    ...
    getAvailableTemplates(niveau, matiere) {
    ...
    generateExercisesBatchForTests(niveau, matiere, difficulte = 'entrainement', count = 5, concepts) {
        const relevantTemplates = Array.from(this.templates.values()).filter(template => template.niveau === niveau &&
            template.matiere === matiere &&
            template.difficulte === difficulte &&
            (!concepts || concepts.some(concept => template.concepts.includes(concept))));
    ...
    generatePersonalizedExercises(studentId, niveau, matiere, weakConcepts = [], count = 3) {
        ...
        const difficulties = ['decouverte', 'entrainement', 'maitrise'];
        const exercises = [];
        for (let i = 0; i < count; i++) {
...
// Singleton instance
exports.exerciseGeneratorService = new ExerciseGeneratorService();
