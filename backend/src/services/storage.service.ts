...
export class StorageService {
    ...
    try {
      const transaction = await db.transaction(async (tx) => {
        ...
        await tx.insert(files).values({
          // id is auto-incremented, don't specify it
          ...
          uploadedBy: parseInt(file.uploadedBy, 10),
          uploadedAt: file.uploadedAt instanceof Date ? file.uploadedAt : new Date(file.uploadedAt),
          category: file.category,
          isPublic: file.isPublic,
          status: file.status,
          checksum: file.checksum
          // createdAt is auto-generated by schema defaultNow()
        });

        // Get the inserted file ID (MySQL doesn't support returning)
        const insertResult = await db.execute(sql`SELECT LAST_INSERT_ID() as id`);
        const newFileId = (insertResult as any[])[0]?.id;
        ...
        if (file.processedVariants && file.processedVariants.length > 0) {
          const variantValues = file.processedVariants.map(variant => ({
            // id is auto-incremented, don't specify it
            fileId: newFileId,
            variant: variant.type, // Use type as variant name
            type: variant.type,
  ...
  async getFilesByUser(
    ...
    try {
      ...
      let whereConditions = eq(files.uploadedBy, parseInt(userId, 10));
