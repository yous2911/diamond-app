...
class GdprService {
  ...
  async hardDeleteStudentData(studentId: number): Promise<{ success: boolean; affectedRecords: number }> {
    ...
    await this.db.delete(gdprFiles).where(eq(gdprFiles.studentId, studentId));
    affectedRecords += 1; // Assume one record was affected

    await this.db.delete(revisions).where(eq(revisions.studentId, studentId));
    affectedRecords += 1; // Assume one record was affected

    await this.db.delete(sessions).where(eq(sessions.studentId, studentId));
    affectedRecords += 1; // Assume one record was affected

    await this.db.delete(studentProgress).where(eq(studentProgress.studentId, studentId));
    affectedRecords += 1; // Assume one record was affected
