...
class FileValidationService {
     ...
     */
    static validateFile(buffer, originalName, declaredMimeType, maxSize) {
        const result = {
            isValid: true,
            errors: [],
            warnings: []
     ...
     */
    static validateMagicBytes(buffer, expectedExtension, declaredMimeType) {
        const result = {
            isValid: false,
            errors: [],
            warnings: []
                ...
                for (const signatureBytes of signature.signatures) {
                    if (this.matchesSignature(buffer, signatureBytes)) {
                        ...
                        if (declaredMimeType !== signature.mimeType) {
                            result.warnings.push(`MIME type déclaré (${declaredMimeType}) ne correspond pas au type détecté (${signature.mimeType})`);
                        }
                        return result;
                    }
                }
     ...
     */
    static checkDangerousSignatures(buffer) {
        const result = {
            isValid: true,
            errors: [],
            warnings: []
     ...
     */
    static performTypeSpecificValidation(buffer, extension) {
        const result = {
            isValid: true,
            errors: [],
            warnings: []
...
FileValidationService.MAGIC_SIGNATURES = [
    // Images JPEG
    {
        extension: 'jpg',
        mimeType: 'image/jpeg',
...
FileValidationService.DANGEROUS_SIGNATURES = [
    [0x4D, 0x5A], // PE/EXE
