...
export class CacheWarmingService {
  ...
  constructor(fastify: FastifyInstance, advancedCache: AdvancedCacheService) {
  ...
  private async warmCoreCompetences(): Promise<void> {
    try {
  ...
  private async warmPopularExercises(): Promise<void> {
    try {
  ...
  private async warmStudentProgressions(): Promise<void> {
    try {
  ...
  private async warmCurriculumLevels(): Promise<void> {
    try {
      ...
      const levelPromises = levels.map(async (level: any) => {
  ...
  private async warmExerciseStats(): Promise<void> {
    try {
      ...
      const individualPromises = stats.map(async (stat: any) => {
  ...
  private async warmAuthenticationData(): Promise<void> {
    try {
  ...
  // PUBLIC METHODS
  async startWarmingScheduler(): Promise<void> {
  ...
  async executeCriticalWarming(strategies?: WarmingStrategy[]): Promise<void> {
    ...
    const results = await Promise.allSettled(
      strategiesToExecute.map(strategy => strategy.execute())
    );
  ...
  async enableStrategy(name: string): Promise<boolean> {
  ...
  async disableStrategy(name: string): Promise<boolean> {
    const strategy = this.strategies.find(s => s.name === name);
    if (strategy) {
