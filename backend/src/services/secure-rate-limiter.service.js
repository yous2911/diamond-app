...
exports.createSecureAuthRateLimiter = exports.SecureRateLimiterService = void 0;
const logger_1 = require("../utils/logger");
class SecureRateLimiterService {
    constructor() {
        this.memoryStore = new Map();
        this.redis = null; // Redis client si disponible
    ...
    async initializeRedis() {
        try {
            ...
            logger_1.logger.warn('⚠️ Redis not available, using in-memory rate limiting', { error: error.message });
            this.redis = null;
        }
    }
    /**
     ...
     */
    async checkRateLimitRedis(key, config, now) {
    ...
    `;
        const result = await this.redis.eval(luaScript, 1, key, config.maxAttempts, config.windowMs, config.blockDuration || config.windowMs * 2, now);
        return {
            count: result[0],
            resetTime: result[1],
            blocked: result[2] === 1,
     ...
     */
    async checkRateLimitMemory(key, config, now) {
    ...
    cleanupExpiredEntries() {
        if (this.redis)
            return; // Pas besoin si on utilise Redis
        const now = Date.now();
        let cleanedCount = 0;
        ...
        if (cleanedCount > 0) {
            logger_1.logger.debug('Rate limit cleanup completed', {
...
exports.SecureRateLimiterService = SecureRateLimiterService;
/**
 * Middleware factory pour l'authentification
 */
function createSecureAuthRateLimiter(config) {
...
exports.createSecureAuthRateLimiter = createSecureAuthRateLimiter;
