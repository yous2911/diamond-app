...
exports.performanceBenchmarkService = void 0;
...
class PerformanceBenchmarkService {
    constructor() {
        ...
        this.config = {
            ...
            outputPath: process.env.BENCHMARK_OUTPUT_PATH || (0, path_1.join)(process.cwd(), 'benchmark-results'),
            ...
            concurrentUsers: [1, 5, 10, 25, 50].map(n => parseInt(process.env[`BENCHMARK_CONCURRENT_${n}`] || n.toString())),
            testDataSizes: [100, 1000, 10000].map(n => parseInt(process.env[`BENCHMARK_DATA_SIZE_${n}`] || n.toString())),
            ...
            tests: [
                {
                    ...
                    execute: async () => {
                        const [rows] = await connection_1.connection.execute('SELECT COUNT(*) as count FROM students');
                        return rows;
                    }
                },
                {
                    ...
                    execute: async () => {
                        return await optimized_queries_1.optimizedQueries.getStudentWithProgress(1);
                    }
                },
                {
                    ...
                    setup: async () => {
                        await connection_1.connection.execute('CREATE TEMPORARY TABLE benchmark_test (id INT AUTO_INCREMENT PRIMARY KEY, data VARCHAR(255))');
                    },
                    execute: async () => {
                        const testData = `test-data-${Date.now()}-${Math.random()}`;
                        const [result] = await connection_1.connection.execute('INSERT INTO benchmark_test (data) VALUES (?)', [testData]);
                        return result;
                    },
                    teardown: async () => {
                        await connection_1.connection.execute('DROP TEMPORARY TABLE IF EXISTS benchmark_test');
                    }
                },
                {
                    id: 'bulk_insert',
                    ...
                    setup: async () => {
                        await connection_1.connection.execute('CREATE TEMPORARY TABLE benchmark_bulk (id INT AUTO_INCREMENT PRIMARY KEY, data VARCHAR(255))');
                    },
                    execute: async () => {
                        const values = Array.from({ length: 100 }, (_, i) => [`test-${i}-${Date.now()}`]);
                        const placeholders = values.map(() => '(?)').join(',');
                        const [result] = await connection_1.connection.execute(`INSERT INTO benchmark_bulk (data) VALUES ${placeholders}`, values.flat());
                        return result;
                    },
                    teardown: async () => {
                        await connection_1.connection.execute('DROP TEMPORARY TABLE IF EXISTS benchmark_bulk');
                    }
                },
                {
                    ...
                    setup: async () => {
                        await connection_1.connection.execute('CREATE TEMPORARY TABLE benchmark_update (id INT AUTO_INCREMENT PRIMARY KEY, data VARCHAR(255), counter INT DEFAULT 0)');
                        await connection_1.connection.execute('INSERT INTO benchmark_update (data) VALUES ("test-data")');
                    },
                    execute: async () => {
                        const [result] = await connection_1.connection.execute('UPDATE benchmark_update SET counter = counter + 1, data = ? WHERE id = 1', [`updated-${Date.now()}`]);
                        return result;
                    },
                    teardown: async () => {
                        await connection_1.connection.execute('DROP TEMPORARY TABLE IF EXISTS benchmark_update');
                    }
                },
                {
                    ...
                    setup: async () => {
                        await connection_1.connection.execute('CREATE TEMPORARY TABLE benchmark_delete (id INT AUTO_INCREMENT PRIMARY KEY, data VARCHAR(255))');
                        await connection_1.connection.execute('INSERT INTO benchmark_delete (data) VALUES ("to-delete")');
                    },
                    execute: async () => {
                        const [result] = await connection_1.connection.execute('DELETE FROM benchmark_delete WHERE id = 1');
                        await connection_1.connection.execute('INSERT INTO benchmark_delete (data) VALUES ("to-delete")'); // Reset for next iteration
                        return result;
                    },
                    teardown: async () => {
                        await connection_1.connection.execute('DROP TEMPORARY TABLE IF EXISTS benchmark_delete');
                    }
                }
            ]
        });
        ...
        this.suites.set('query_optimization', {
            ...
            tests: [
                ...
                },
                {
                    ...
                    execute: async () => {
                        return await optimized_queries_1.optimizedQueries.getExercisesWithStats({ matiere: 'maths' }, { limit: 20 });
                    },
                ...
                },
                {
                    ...
                    execute: async () => {
                        return await optimized_queries_1.optimizedQueries.getLearningAnalytics();
                    },
                ...
                },
                {
                    ...
                    execute: async () => {
                        return await optimized_queries_1.optimizedQueries.getRecommendedExercises(1, 10);
                    },
    ...
    async initialize() {
        if (!this.config.enabled) {
            logger_1.logger.info('Performance benchmarking disabled');
            return;
        }
        try {
            logger_1.logger.info('Initializing performance benchmark service...');
            // Ensure output directory exists
            await fs_1.promises.mkdir(this.config.outputPath, { recursive: true });
            // Load existing baselines if available
            await this.loadBaselines();
            logger_1.logger.info('Performance benchmark service initialized', {
                suites: this.suites.size,
                outputPath: this.config.outputPath
            });
        }
        catch (error) {
            logger_1.logger.error('Failed to initialize performance benchmark service', { error });
            throw error;
        }
    }
    async loadBaselines() {
        try {
            const baselinePath = (0, path_1.join)(this.config.outputPath, 'baselines.json');
            try {
                const content = await fs_1.promises.readFile(baselinePath, 'utf8');
                ...
                logger_1.logger.info('Loaded performance baselines', { count: this.baselines.size });
            }
            catch (error) {
                logger_1.logger.debug('No existing baselines found');
            }
        }
        catch (error) {
            logger_1.logger.error('Failed to load baselines', { error });
        }
    }
    async saveBaselines() {
        try {
            ...
            await fs_1.promises.writeFile(baselinePath, JSON.stringify(baselines, null, 2));
            logger_1.logger.info('Baselines saved', { count: this.baselines.size });
        }
        catch (error) {
            logger_1.logger.error('Failed to save baselines', { error });
        }
    }
    async runBenchmark(suiteId, options = {}) {
        if (this.isRunning) {
        ...
        try {
            logger_1.logger.info('Starting performance benchmark', {
                suiteId: suiteId || 'all',
            ...
            for (const suite of suitesToRun) {
                if (!suite)
                    continue;
                logger_1.logger.info('Running benchmark suite', { suiteName: suite.name });
                for (const test of suite.tests) {
                    try {
                        ...
                        logger_1.logger.error('Benchmark test failed', {
                            testId: test.id,
                            error: error.message
                        });
                    }
    ...
    async runTest(test, category, options) {
        ...
        try {
            ...
            // Warmup
            logger_1.logger.debug('Warming up test', { testId: test.id });
            for (let i = 0; i < this.config.warmupIterations; i++) {
                try {
                    await Promise.race([
                        test.execute(),
                        new Promise((_, reject) => setTimeout(() => reject(new Error('Warmup timeout')), this.config.timeoutMs))
                    ]);
                }
                catch (error) {
            ...
            // Actual benchmark
            logger_1.logger.debug('Running benchmark test', { testId: test.id, iterations });
            const executions = [];
            ...
            logger_1.logger.error('Test execution failed', { testId: test.id, error });
            throw error;
        }
    }
    async executeTestIteration(test, iteration) {
        const startTime = Date.now();
        const startMemory = process.memoryUsage().heapUsed;
        try {
            const result = await Promise.race([
                test.execute(),
                new Promise((_, reject) => setTimeout(() => reject(new Error('Test timeout')), this.config.timeoutMs))
            ]);
    ...
    calculateStatistics(executionTimes, totalExecutions) {
    ...
    async generateReport(results, systemInfo) {
    ...
    generateRecommendations(results, comparisons) {
        const recommendations = [];
        // Analyze slow queries
        const slowQueries = results.filter(r => r.category === 'database' && r.statistics.mean > 1000);
        if (slowQueries.length > 0) {
            recommendations.push(`⚠️ ${slowQueries.length} database queries are taking longer than 1 second. Consider optimization.`);
        }
        // Analyze error rates
        const highErrorRates = results.filter(r => r.statistics.errorRate > 5);
        if (highErrorRates.length > 0) {
            recommendations.push(`🚨 ${highErrorRates.length} tests have error rates above 5%. Investigate connection issues.`);
        }
        // Analyze degraded performance
        const degradedTests = comparisons.filter(c => c.status === 'degraded');
        if (degradedTests.length > 0) {
            recommendations.push(`📉 ${degradedTests.length} tests show performance degradation compared to baseline.`);
        }
        // Memory recommendations
        const memoryIntensiveTests = results.filter(r => r.results.some(exec => (exec.memoryUsed || 0) > 10 * 1024 * 1024) // 10MB
        );
        if (memoryIntensiveTests.length > 0) {
            recommendations.push(`💾 ${memoryIntensiveTests.length} tests are memory-intensive. Monitor for memory leaks.`);
        }
        ...
        if (overallScore < 10) { // Less than 10 QPS average
            recommendations.push('⚡ Overall throughput is low. Consider database indexing and query optimization.');
        }
    ...
    async getSystemInfo() {
        ...
        try {
            const [rows] = await connection_1.connection.execute('SELECT VERSION() as version');
            databaseVersion = rows[0]?.version || 'Unknown';
        }
        catch (error) {
    ...
    async saveReport(report) {
        const reportPath = (0, path_1.join)(this.config.outputPath, `benchmark-report-${report.id}.json`);
        await fs_1.promises.writeFile(reportPath, JSON.stringify(report, null, 2));
        logger_1.logger.info('Benchmark report saved', { reportPath });
    }
    // Public API methods
    ...
    async setBaseline() {
        ...
        logger_1.logger.info('New performance baseline established');
    }
    getResults(testId) {
        if (testId) {
...
exports.performanceBenchmarkService = new PerformanceBenchmarkService();
exports.default = exports.performanceBenchmarkService;
