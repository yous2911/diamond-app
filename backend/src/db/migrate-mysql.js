...
class MySQLMigrationRunner {
    ...
    async connect() {
        try {
            ...
            logger_1.logger.info('Connected to MySQL server', {
                host: config_1.dbConfig.host,
                port: config_1.dbConfig.port,
                user: config_1.dbConfig.user
            });
            ...
            await this.connection.execute(`USE ${config_1.dbConfig.database}`);
            logger_1.logger.info('Switched to database', { database: config_1.dbConfig.database });
        }
        catch (error) {
    ...
    async createDatabase() {
        try {
            await this.connection.execute(`CREATE DATABASE IF NOT EXISTS ${config_1.dbConfig.database}`);
            logger_1.logger.info('Database created or already exists', { database: config_1.dbConfig.database });
        }
        catch (error) {
    ...
    async verifySetup() {
        try {
            logger_1.logger.info('Verifying database setup...');
      ...
      `, [config_1.dbConfig.database]);
            ...
            if (missingTables.length > 0) {
                logger_1.logger.error('Missing tables:', missingTables);
      ...
      `);
            logger_1.logger.info('Database verification completed', {
                tables: tableNames.length,
                students: studentCount[0].count,
                competences: competenceCount[0].count
            });
        }
        catch (error) {
 ...
 */
async function runMigrations(includeSampleData = true) {
    const migrationRunner = new MySQLMigrationRunner();
    try {
        logger_1.logger.info('Starting MySQL database migration...');
        ...
        logger_1.logger.info('MySQL database migration completed successfully!');
    }
    catch (error) {
