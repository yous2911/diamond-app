...
const file_security_middleware_1 = require("../middleware/file-security.middleware");
const logger_1 = require("../utils/logger");
// Request schemas
const metricsQuerySchema = zod_1.z.object({
    format: zod_1.z.enum(['json', 'prometheus']).default('json'),
    include: zod_1.z.array(zod_1.z.enum(['system', 'database', 'cache', 'security'])).optional()
});
...
const monitoringRoutes = async (fastify) => {
    const shutdownService = (0, graceful_shutdown_service_1.getGracefulShutdownService)();
        ...
        handler: async (request, reply) => {
            try {
                ...
                if (shouldIncludeAll || include?.includes('security')) {
                    try {
                        const securityStats = file_security_middleware_1.FileSecurityMiddleware.getSecurityStats();
                        metrics.security = {
    ...
    logger_1.logger.info('✅ Enhanced monitoring routes registered successfully');
};
/**
