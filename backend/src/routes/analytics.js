...
async function analyticsRoutes(fastify) {
    ...
    fastify.get('/student/:studentId/progress', {
        ...
        try {
            ...
            const progress = await analytics_service_1.analyticsService.getStudentAnalytics(studentId);
            return reply.send({
                success: true,
                data: progress
            });
    ...
    fastify.get('/student/:studentId/sessions', {
        ...
        try {
            ...
            const stats = await analytics_service_1.analyticsService.getLearningInsights('month');
            return reply.send({
                success: true,
                data: stats
            });
    ...
    fastify.get('/exercise/:exerciseId/completion', {
        ...
        try {
            ...
            const completion = await analytics_service_1.analyticsService.getLearningInsights('month');
            return reply.send({
                success: true,
                data: completion
            });
    ...
    fastify.get('/comprehensive', {
        ...
        try {
            const { studentId, startDate, endDate, matiere, groupBy, limit, offset } = request.query;
            // Get analytics data using the new service
            const analyticsData = await enhanced_database_service_js_1.enhancedDatabaseService.getDailyLearningAnalytics({
                studentId,
                startDate,
                endDate,
                matiere,
    ...
    fastify.get('/sessions', {
        ...
        try {
            const { studentId, startDate, endDate, limit, offset } = request.query;
            // Get session data using the new service
            const sessionData = await enhanced_database_service_js_1.enhancedDatabaseService.getLearningSessionTracking({
                studentId,
                startDate,
                endDate,
                limit,
