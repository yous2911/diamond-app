...
const LeaderboardQuerySchema = zod_1.z.object({
    type: zod_1.z.enum(['global', 'class', 'weekly', 'monthly']).default('global'),
    category: zod_1.z.enum(['points', 'streak', 'exercises', 'accuracy']).default('points'),
    limit: zod_1.z.coerce.number().min(1).max(100).default(50),
    classId: zod_1.z.coerce.number().optional(),
    period: zod_1.z.string().optional()
});
const StudentRankQuerySchema = zod_1.z.object({
    type: zod_1.z.enum(['global', 'class', 'weekly', 'monthly']).default('global'),
    category: zod_1.z.enum(['points', 'streak', 'exercises', 'accuracy']).default('points')
});
const NearbyCompetitorsSchema = zod_1.z.object({
    type: zod_1.z.enum(['global', 'class', 'weekly', 'monthly']).default('global'),
    category: zod_1.z.enum(['points', 'streak', 'exercises', 'accuracy']).default('points'),
    range: zod_1.z.coerce.number().min(1).max(20).default(5)
});
async function leaderboardRoutes(fastify) {
    ...
    fastify.get('/api/leaderboards', {
        ...
        try {
            ...
            const result = await leaderboard_service_1.leaderboardService.getLeaderboard(query.type, query.category, {
                limit: query.limit,
            ...
            fastify.log.error({ err }, 'Error fetching leaderboard');
            reply.code(500).send({
                success: false,
    ...
    fastify.get('/api/leaderboards/student/:studentId/rank', {
        ...
        try {
            ...
            const rank = await leaderboard_service_1.leaderboardService.getStudentRank(studentId, query.type, query.category);
            if (!rank) {
                reply.code(404).send({
                    success: false,
            ...
            fastify.log.error('Error fetching student rank:', error);
            reply.code(500).send({
                success: false,
    ...
    fastify.get('/api/leaderboards/user-centric/:studentId', {
        ...
        try {
            ...
            const userCentricView = await leaderboard_service_1.leaderboardService.getUserCentricLeaderboard(studentId, query.type, query.category, query.range);
            reply.send({
                success: true,
            ...
            fastify.log.error('Error fetching user-centric leaderboard:', error);
            reply.code(500).send({
                success: false,
    ...
    fastify.get('/api/leaderboards/student/:studentId/competitors', {
        ...
        try {
            ...
            const competitors = await leaderboard_service_1.leaderboardService.getNearbyCompetitors(studentId, query.type, query.category, query.range);
            reply.send({
                success: true,
                data: competitors,
            ...
            fastify.log.error('Error fetching competitors:', error);
            reply.code(500).send({
                success: false,
    ...
    fastify.get('/api/badges/student/:studentId', {
        ...
        try {
            ...
            fastify.log.error('Error fetching student badges:', error);
            reply.code(500).send({
                success: false,
    ...
    fastify.post('/api/leaderboards/update', {
        ...
        try {
            ...
            fastify.log.error('Error updating leaderboards:', error);
            reply.code(500).send({
                success: false,
    ...
    fastify.get('/api/leaderboards/stats', {
        ...
        try {
            ...
            fastify.log.error('Error fetching leaderboard stats:', error);
            reply.code(500).send({
                success: false,
    ...
    fastify.get('/api/competitions', {
        ...
        try {
            ...
            fastify.log.error('Error fetching competitions:', error);
            reply.code(500).send({
                success: false,
    ...
    fastify.log.info('✅ Leaderboard routes registered successfully');
}
