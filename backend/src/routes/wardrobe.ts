  ...
  fastify.get('/:studentId', {
    ...
    handler: async (request: FastifyRequest<{
      ...
      try {
        ...
        const whereConditions = [eq(wardrobeItems.isActive, true)];
        ...
        const allItems = await baseQuery.where(and(...whereConditions));
      ...
      try {
        ...
        for (const sameTypeItem of sameTypeItems) {
          await db
            ...
            .where(eq(studentWardrobe.id, sameTypeItem.id));
        }
