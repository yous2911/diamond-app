...
// Mock the files table in schema
vi.mock('../db/schema', async () => {
  const original = await vi.importActual('../db/schema');
  return {
    ...original,
    files: {
      id: 'id',
...
describe('File Upload System Tests', () => {
  ...
  afterAll(async () => {
    ...
    // Cleanup test files with retry for Windows file locking issues
  ...
  async function cleanupTestFiles(): Promise<void> {
    ...
    for (let i = 0; i < maxRetries; i++) {
      try {
        if (await fs.pathExists(testFilesDir)) {
          await fs.remove(testFilesDir);
        }
        return;
      } catch (error: any) {
        if (error.code === 'EBUSY' && i < maxRetries - 1) {
          console.warn(`Cleanup attempt ${i + 1} failed, retrying in ${delay}ms...`);
