...
describe('EnhancedRateLimitingService', () => {
  ...
  describe('Geographic Rate Limiting', () => {
    it('should apply geographic limits', async () => {
      ...
      require('geoip-lite').lookup = vi.fn().mockReturnValue({ country: 'CN' });
  ...
  describe('Statistics and Monitoring', () => {
    ...
    it('should track suspicious IPs', async () => {
      ...
      expect(stats.topSuspiciousIPs[0]).toBeDefined();
      expect(Array.isArray(stats.topSuspiciousIPs[0])).toBe(true);
      expect(stats.topSuspiciousIPs[0][0]).toBeDefined(); // IP
      expect(stats.topSuspiciousIPs[0][1]).toBeGreaterThan(0); // Score
  ...
  describe('Error Handling', () => {
    it('should handle middleware errors gracefully', async () => {
      ...
      (service as any).checkGlobalLimit = vi.fn().mockRejectedValue(new Error('Test error'));
