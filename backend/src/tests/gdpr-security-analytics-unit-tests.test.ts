...
describe('GDPR, Security & Analytics Unit Tests', () => {
  let mockDb: any;
  ...
  describe('Analytics Service Functions', () => {
    ...
    test('should calculate learning streak', () => {
      ...
      expect(calculateStreak([true, true, true, false, true])).toBe(0);
    ...
    test('should validate timeframe calculations', () => {
      ...
      expect(now.getTime() - yearAgo.getTime()).toBeGreaterThan(365 * 24 * 60 * 60 * 1000);
    });
    ...
    test('should calculate performance metrics', () => {
      ...
      expect(metrics.standardDeviation).toBe(5.35);
    });
