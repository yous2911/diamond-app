...
vitest_1.vi.mock('../services/service-factory', () => {
    const mockEncryptionService = {
        encrypt: vitest_1.vi.fn((data) => Promise.resolve({
            encryptedData: Buffer.from(JSON.stringify(data)).toString('base64'),
            keyId: 'test-key-123',
            algorithm: 'AES-256-GCM'
        })),
        decrypt: vitest_1.vi.fn((encryptedData) => Promise.resolve({
            decryptedData: JSON.parse(Buffer.from(encryptedData, 'base64').toString()),
            keyId: 'test-key-123'
        })),
        encryptStudentData: vitest_1.vi.fn((data) => Promise.resolve({
            encryptedData: Buffer.from(JSON.stringify(data)).toString('base64'),
            keyId: 'test-key-123',
            algorithm: 'AES-256-GCM',
            timestamp: new Date().toISOString()
        })),
        decryptStudentData: vitest_1.vi.fn((encryptedData) => {
            try {
        ...
        generateSHA256Hash: vitest_1.vi.fn((data) => {
            const hash = Buffer.from(data + 'mock-salt').toString('hex').substring(0, 64);
            return hash;
        }),
        generateSecureToken: vitest_1.vi.fn(() => {
            return 'mock-secure-token-' + Math.random().toString(36).substring(2, 15);
        }),
        generateIntegrityChecksum: vitest_1.vi.fn((_data) => {
            return 'a'.repeat(64);
        }),
        verifyIntegrityChecksum: vitest_1.vi.fn((_data, checksum) => {
            return checksum.length === 64;
        }),
        generateDigitalSignature: vitest_1.vi.fn((_data) => {
            return 'b'.repeat(64);
        }),
        verifyDigitalSignature: vitest_1.vi.fn((_data, signature) => {
            return signature.length === 64;
        }),
        generateSalt: vitest_1.vi.fn(() => {
            return Buffer.from('mock-salt-' + Math.random().toString(36).substring(2, 15));
        }),
        deriveEncryptionKey: vitest_1.vi.fn((_password, _salt) => {
            const combined = _password + _salt.toString();
            return Buffer.from(combined + 'derived-key').subarray(0, 32);
        }),
        deriveKeyPBKDF2: vitest_1.vi.fn((_password, _salt, _iterations, keyLength) => {
            return Buffer.alloc(keyLength, 0x42);
        }),
        secureCompare: vitest_1.vi.fn((a, b) => {
            return a === b;
        }),
        getEncryptionStats: vitest_1.vi.fn(() => ({
        ...
        getKeyInfo: vitest_1.vi.fn((keyId) => {
        ...
        testEncryptionService: vitest_1.vi.fn(async () => {
            const testData = { test: 'data' };
            const encrypted = await mockEncryptionService.encryptStudentData(testData);
            const decrypted = await mockEncryptionService.decryptStudentData(encrypted);
            return {
                success: true,
                tests: [
    ...
    const mockEmailService = {
        sendEmail: vitest_1.vi.fn(() => Promise.resolve()),
        sendBulkEmail: vitest_1.vi.fn(() => Promise.resolve()),
        sendTemplateEmail: vitest_1.vi.fn(() => Promise.resolve()),
        validateEmail: vitest_1.vi.fn(() => Promise.resolve(true)),
        getEmailTemplates: vitest_1.vi.fn(() => Promise.resolve([])),
        createEmailTemplate: vitest_1.vi.fn(() => Promise.resolve()),
        updateEmailTemplate: vitest_1.vi.fn(() => Promise.resolve()),
        deleteEmailTemplate: vitest_1.vi.fn(() => Promise.resolve()),
        getAvailableTemplates: vitest_1.vi.fn(() => [
        ...
        validateTemplateVariables: vitest_1.vi.fn((_template, variables) => {
        ...
        sendUserRegistrationWelcome: vitest_1.vi.fn(() => Promise.resolve()),
        sendUserRegistrationVerification: vitest_1.vi.fn(() => Promise.resolve()),
        sendPasswordResetRequest: vitest_1.vi.fn(() => Promise.resolve()),
        sendPasswordResetConfirmation: vitest_1.vi.fn(() => Promise.resolve()),
        sendStudentProgressReport: vitest_1.vi.fn(() => Promise.resolve()),
        sendAchievementNotification: vitest_1.vi.fn(() => Promise.resolve()),
        sendMaintenanceNotification: vitest_1.vi.fn(() => Promise.resolve()),
        sendSecurityAlert: vitest_1.vi.fn(() => Promise.resolve()),
        sendBulkEmails: vitest_1.vi.fn(() => Promise.resolve({
            sent: 2,
            failed: 0,
            errors: []
        })),
        getEmailServiceStatus: vitest_1.vi.fn(() => ({
            status: 'healthy',
            smtpConnected: true,
            templatesLoaded: true,
            lastSent: new Date()
        })),
        sendTestEmail: vitest_1.vi.fn(() => Promise.resolve(true)),
        sendEmailWithRetry: vitest_1.vi.fn(() => Promise.resolve())
    };
    const mockAuditService = {
        logAction: vitest_1.vi.fn(() => Promise.resolve('mock-audit-id-123')),
        queryAuditLogs: vitest_1.vi.fn(() => Promise.resolve({
            entries: [{
        ...
        generateComplianceReport: vitest_1.vi.fn(() => Promise.resolve({
            id: 'mock-report-123',
            success: true,
            filters: {},
            exportFormat: 'json',
        ...
        getStudentAuditTrail: vitest_1.vi.fn(() => Promise.resolve([{
                id: 'mock-trail-123',
                entityType: 'student',
                action: 'read',
                timestamp: new Date(),
        ...
        anonymizeStudentAuditLogs: vitest_1.vi.fn(() => Promise.resolve({ count: 5 })),
        verifyAuditIntegrity: vitest_1.vi.fn(() => Promise.resolve({
            valid: true,
            tampering: false,
            originalChecksum: 'mock-checksum',
            calculatedChecksum: 'mock-checksum'
        })),
        detectSecurityAnomalies: vitest_1.vi.fn(() => Promise.resolve({
            entityId: 'test-entity',
            anomalies: []
        })),
        cleanupOldAuditLogs: vitest_1.vi.fn(() => Promise.resolve({ deleted: 0 }))
    };
    const mockStorageService = {
        uploadFile: vitest_1.vi.fn(() => Promise.resolve({
            success: true,
            filePath: '/uploads/test-file.jpg'
        })),
        deleteFile: vitest_1.vi.fn(() => Promise.resolve({
            success: true
        })),
        getFileUrl: vitest_1.vi.fn(() => Promise.resolve('https://example.com/file.jpg')),
        listFiles: vitest_1.vi.fn(() => Promise.resolve([])),
        saveFileMetadata: vitest_1.vi.fn((fileMetadata) => Promise.resolve({
            success: true,
            id: fileMetadata.id || 'test-file-123'
        })),
        getFileById: vitest_1.vi.fn((id) => Promise.resolve({
            id,
        ...
        findFileByChecksum: vitest_1.vi.fn((_checksum) => Promise.resolve(null)),
        getStorageStats: vitest_1.vi.fn(() => Promise.resolve({
            totalFiles: 10,
            totalSize: 1024000,
        ...
        cleanupExpiredFiles: vitest_1.vi.fn((_daysOld) => Promise.resolve(5)),
        getStorageHealth: vitest_1.vi.fn(() => Promise.resolve({
            status: 'healthy',
            diskUsage: 0.1,
            availableSpace: 9000000000,
            issues: [],
    ...
    const mockFileSecurityService = {
        ...
        validateFile: vitest_1.vi.fn((buffer, filename, mimeType) => {
            const dangerousExtensions = ['.exe', '.bat', '.cmd', '.scr', '.pif'];
        ...
        performSecurityScan: vitest_1.vi.fn((_filePath, buffer) => {
        ...
        validateFileType: vitest_1.vi.fn((file) => {
        ...
        checkFileSize: vitest_1.vi.fn((file, maxSize = 5 * 1024 * 1024) => {
            return Promise.resolve({
    ...
    const mockImageProcessingService = {
        processImage: vitest_1.vi.fn(() => Promise.resolve({
            success: true,
            processedPath: '/processed/image.jpg'
        })),
        optimizeImage: vitest_1.vi.fn(() => Promise.resolve({
            success: true,
            optimizedPath: '/optimized/image.jpg'
        })),
        getImageInfo: vitest_1.vi.fn((buffer) => {
            let format = 'jpeg';
            let width = 800;
            let height = 600;
        ...
        validateImageStructure: vitest_1.vi.fn((buffer) => {
        ...
        generateThumbnails: vitest_1.vi.fn(async (_imagePath, sizes) => {
            const results = [];
        ...
        compressImage: vitest_1.vi.fn((buffer, options) => {
        ...
        addWatermark: vitest_1.vi.fn((buffer, _watermarkOptions) => {
            return Promise.resolve(Buffer.concat([buffer, Buffer.from('WATERMARK')]));
        }),
        convertFormat: vitest_1.vi.fn((buffer, format, _quality) => {
            return Promise.resolve(Buffer.concat([buffer, Buffer.from(format.toUpperCase())]));
        }),
        optimizeForWeb: vitest_1.vi.fn((buffer) => {
            return Promise.resolve({
        ...
        resizeImage: vitest_1.vi.fn((buffer, options, _qualityOptions) => {
    ...
    return {
        ServiceFactory: {
            getEncryptionService: vitest_1.vi.fn(() => mockEncryptionService),
            getEmailService: vitest_1.vi.fn(() => mockEmailService),
            getAuditTrailService: vitest_1.vi.fn(() => mockAuditService),
            getStorageService: vitest_1.vi.fn(() => mockStorageService),
            getFileSecurityService: vitest_1.vi.fn(() => mockFileSecurityService),
            getImageProcessingService: vitest_1.vi.fn(() => mockImageProcessingService),
            clearInstances: vitest_1.vi.fn(),
            setMockInstance: vitest_1.vi.fn()
        }
    };
});
// Mock Fastify instance methods - we'll add authenticate method after app is built
// Mock optimized queries
vitest_1.vi.mock('../db/optimized-queries', () => ({
    getStudentWithProgress: vitest_1.vi.fn((studentId) => {
        if (studentId === 1) {
            return Promise.resolve({
                id: 1,
                prenom: 'Alice',
    ...
    getRecommendedExercises: vitest_1.vi.fn((_studentId, _limit = 10) => {
        return Promise.resolve([
            {
                id: 1,
                title: 'Test Exercise',
...
vitest_1.vi.mock('../db/connection', () => {
    ...
    const mockDb = {
        select: vitest_1.vi.fn(() => ({
            from: vitest_1.vi.fn(() => ({
                limit: vitest_1.vi.fn(() => Promise.resolve(mockStudents)),
                where: vitest_1.vi.fn(() => ({
                    limit: vitest_1.vi.fn(() => Promise.resolve([{
                            id: 'mock-audit-id-123',
                            entityType: 'student',
                            entityId: 'test-student-123',
                            action: 'read',
        ...
        insert: vitest_1.vi.fn(() => ({
            values: vitest_1.vi.fn(() => Promise.resolve([{ insertId: 1 }]))
        })),
        ...
        execute: vitest_1.vi.fn(() => Promise.resolve([{ test: 1 }])),
        transaction: vitest_1.vi.fn(async (callback) => {
            const tx = {
                insert: vitest_1.vi.fn(() => ({
                    values: vitest_1.vi.fn(() => Promise.resolve({
    ...
    return {
        ...
        reconnectDatabase: vitest_1.vi.fn(() => Promise.resolve(true)),
        getPoolStats: vitest_1.vi.fn(() => Promise.resolve({
            activeConnections: 0,
            totalConnections: 10,
            idleConnections: 10,
            queuedRequests: 0
        ...
        connection: {
            execute: vitest_1.vi.fn(() => Promise.resolve([{ test: 1 }])),
            getConnection: vitest_1.vi.fn(() => Promise.resolve({
                beginTransaction: vitest_1.vi.fn(),
                commit: vitest_1.vi.fn(),
                rollback: vitest_1.vi.fn(),
                release: vitest_1.vi.fn(),
                execute: vitest_1.vi.fn(() => Promise.resolve([]))
            }))
        }
    };
});
vitest_1.vi.mock('../db/setup', () => ({
    setupDatabase: vitest_1.vi.fn(() => {
        console.log('🧪 Using mock database for tests');
        return Promise.resolve();
    }),
    resetDatabase: vitest_1.vi.fn(() => Promise.resolve()),
    checkDatabaseSetup: vitest_1.vi.fn(() => Promise.resolve({
        isSetup: true,
        studentCount: 1,
        exerciseCount: 1
    }))
...
vitest_1.vi.mock('../services/data-anonymization.service', () => {
    const mockAnonymizationService = {
        anonymizeStudentData: vitest_1.vi.fn((studentData) => {
        ...
        deleteStudentData: vitest_1.vi.fn(() => Promise.resolve({ success: true })),
        scheduleDataDeletion: vitest_1.vi.fn(() => Promise.resolve()),
        getRetentionStatus: vitest_1.vi.fn(() => Promise.resolve({ status: 'active' })),
        processRetentionRequests: vitest_1.vi.fn(() => Promise.resolve()),
        generateDataReport: vitest_1.vi.fn(() => Promise.resolve({ students: [], summary: {} })),
        // Missing methods that were causing errors
        canReverseAnonymization: vitest_1.vi.fn(() => Promise.resolve(true)),
        getAnonymizationReport: vitest_1.vi.fn(() => Promise.resolve({
            totalAnonymized: 5,
    ...
    return {
        DataAnonymizationService: vitest_1.vi.fn().mockImplementation(() => mockAnonymizationService),
        // Export the service instance for direct imports
...
const mockAuditService = vitest_1.vi.hoisted(() => ({
    logAction: vitest_1.vi.fn(() => Promise.resolve('mock-audit-id-123')),
    queryAuditLogs: vitest_1.vi.fn(() => Promise.resolve({
        entries: [{
                id: 'mock-audit-id-123',
...
vitest_1.vi.mock('../services/audit-trail.service', () => ({
    AuditTrailService: vitest_1.vi.fn().mockImplementation(() => mockAuditService),
...
vitest_1.vi.mock('../services/consent.service', () => {
    ...
    const mockConsentService = {
        submitConsentRequest: vitest_1.vi.fn((data) => {
        ...
        verifyConsent: vitest_1.vi.fn((token) => {
        ...
        revokeConsent: vitest_1.vi.fn((token) => {
            const consent = Array.from(consentStorage.values()).find(c => c.token === token);
            if (!consent) {
                return Promise.reject(new Error('Consent not found'));
            }
        ...
        getConsentStatus: vitest_1.vi.fn((token) => {
            const consent = Array.from(consentStorage.values()).find(c => c.token === token);
            if (!consent) {
                return Promise.reject(new Error('Consent not found'));
            }
        ...
        validateConsent: vitest_1.vi.fn((token) => {
            const consent = Array.from(consentStorage.values()).find(c => c.token === token);
            if (!consent) {
        ...
        updateConsentPreferences: vitest_1.vi.fn((preferences) => {
...
vitest_1.vi.mock('../services/data-retention.service', () => {
    const mockDataRetentionService = {
        applyRetentionPolicy: vitest_1.vi.fn(async (studentId) => {
            // Simulate calling audit service
            const auditModule = await Promise.resolve().then(() => __importStar(require('../services/audit-trail.service')));
            const auditService = auditModule.auditTrailService;
            ...
            const anonymizationModule = await Promise.resolve().then(() => __importStar(require('../services/data-anonymization.service')));
            const anonymizationService = anonymizationModule.dataAnonymizationService;
            await anonymizationService.anonymizeStudentData(studentId);
            return { success: true, processed: 1 };
        }),
        scheduleRetentionCheck: vitest_1.vi.fn(() => Promise.resolve({ scheduled: true })),
        getRetentionStatus: vitest_1.vi.fn(() => Promise.resolve({
            totalStudents: 2,
            pendingRetention: 1,
            lastCheck: new Date()
        })),
        // Add missing methods that tests expect
        findRecordsForRetention: vitest_1.vi.fn((_policy) => {
            return Promise.resolve([
                { id: 1, prenom: 'Old', nom: 'Student', lastActivity: new Date(Date.now() - 365 * 24 * 60 * 60 * 1000) }
            ]);
        }),
        processRetentionAction: vitest_1.vi.fn((record, action) => {
            return Promise.resolve({ success: true, action, recordId: record.id });
        }),
        hasRetentionExceptions: vitest_1.vi.fn((record) => {
            return record.metadata?.exceptions?.length > 0 || false;
        }),
        getActivePolicies: vitest_1.vi.fn(() => {
            return Promise.resolve([
        ...
        calculateRetentionStats: vitest_1.vi.fn(() => {
            return Promise.resolve({
                totalRecords: 100,
                pendingRetention: 15,
                processedToday: 5,
        ...
        getRetentionStatistics: vitest_1.vi.fn(() => {
            return Promise.resolve({
                totalRecords: 100,
                pendingRetention: 15,
                processedToday: 5,
    ...
    return {
        DataRetentionService: vitest_1.vi.fn().mockImplementation(() => mockDataRetentionService),
...
const mockFileUploadService = vitest_1.vi.hoisted(() => {
    let fileIdCounter = 1;
    return {
        processFile: vitest_1.vi.fn((file) => {
        ...
        processUpload: vitest_1.vi.fn((uploadRequest, studentId) => {
        ...
        validateFile: vitest_1.vi.fn((file) => {
        ...
        generateUniqueFilename: vitest_1.vi.fn((originalName) => {
...
vitest_1.vi.mock('../services/file-upload.service', () => ({
    FileUploadService: vitest_1.vi.fn().mockImplementation(() => mockFileUploadService),
    fileUploadService: mockFileUploadService,
    default: mockFileUploadService
}));
// Mock File Security Service
const mockFileSecurityService = vitest_1.vi.hoisted(() => ({
    scanFile: vitest_1.vi.fn((file) => {
        const dangerousExtensions = ['.exe', '.bat', '.cmd', '.scr', '.pif'];
        const extension = file.originalname.toLowerCase().substring(file.originalname.lastIndexOf('.'));
        if (dangerousExtensions.includes(extension)) {
...
vitest_1.vi.mock('../services/file-security.service', () => ({
    FileSecurityService: vitest_1.vi.fn().mockImplementation((options) => {
        if (options && options.maxScanTimeMs === 1) {
            return {
                ...mockFileSecurityService,
                performSecurityScan: vitest_1.vi.fn((_h, buffer) => {
...
vitest_1.vi.mock('../services/exercise-generator.service', () => {
    const mockExerciseGeneratorService = {
        generateExercise: vitest_1.vi.fn((type, niveau) => {
            const exercises = {
                'math': {
                    'ce2': {
                        contenu: {
        ...
        generateMathExercise: vitest_1.vi.fn((niveau) => {
            return Promise.resolve({
                contenu: {
                    question: 'Combien font 5 + 3 ?',
                    reponses: ['7', '8', '9', '6'],
        ...
        generateFrenchExercise: vitest_1.vi.fn((niveau) => {
            return Promise.resolve({
                contenu: {
        ...
        generateExercisesBatchForTests: vitest_1.vi.fn((niveau, matiere, difficulte, count = 1) => {
            const exercises = [];
        ...
        generatePersonalizedExercises: vitest_1.vi.fn((studentId, niveau, matiere, _topics, count = 1) => {
            const exercises = [];
            for (let i = 0; i < count; i++) {
                exercises.push({
        ...
        getAvailableTemplates: vitest_1.vi.fn((niveau, matiere) => {
            const templates = [
...
vitest_1.vi.mock('../services/input-sanitization.service', () => {
    const mockInputSanitizationService = {
        sanitizeInput: vitest_1.vi.fn((input) => {
        ...
        validateEmail: vitest_1.vi.fn((email) => {
        ...
        validateUrl: vitest_1.vi.fn((url) => {
            try {
                new URL(url);
                return { valid: true };
            }
        ...
        validateLength: vitest_1.vi.fn((input, maxLength) => {
            return {
...
vitest_1.vi.mock('../middleware/input-sanitization.middleware', () => {
    class MockInputSanitizationService {
        async sanitizationMiddleware(req, reply) {
            ...
            try {
                ...
                const sanitizeValue = (value, key, visited = new Set()) => {
...
vitest_1.vi.mock('../services/security-audit.service', () => {
    const mockSecurityAuditService = {
        // Clear incidents for fresh test state
        clearIncidents: vitest_1.vi.fn(() => {
            securityIncidents.length = 0;
            securityIncidentIdCounter = 1;
        }),
        logIncident: vitest_1.vi.fn((type, severity, component, request, details) => {
        ...
        logManualIncident: vitest_1.vi.fn((type, severity, details = {}) => {
            const id = `SEC-${securityIncidentIdCounter++}-${Math.random().toString(36).substr(2, 9)}`;
            const incident = {
                id,
                type,
        ...
        getIncident: vitest_1.vi.fn((id) => {
            return Promise.resolve(securityIncidents.find(i => i.id === id) || null);
        }),
        getIncidents: vitest_1.vi.fn((filters = {}) => {
            ...
            if (filters.timeRange) {
                filtered = filtered.filter(i => i.timestamp >= filters.timeRange.start &&
        ...
        getMetrics: vitest_1.vi.fn((timeRange = null) => {
            let relevantIncidents = securityIncidents;
            if (timeRange) {
                relevantIncidents = securityIncidents.filter(i => i.timestamp >= timeRange.start && i.timestamp <= timeRange.end);
            }
        ...
        isIPSuspicious: vitest_1.vi.fn((ip, timeWindow = { hours: 24 }) => {
            const cutoff = new Date(Date.now() - timeWindow.hours * 60 * 60 * 1000);
            const ipIncidents = securityIncidents.filter(i => i.source?.ip === ip && i.timestamp >= cutoff);
        ...
        generateReport: vitest_1.vi.fn((timeRange = null) => {
            let relevantIncidents = securityIncidents;
            if (timeRange) {
                relevantIncidents = securityIncidents.filter(i => i.timestamp >= timeRange.start && i.timestamp <= timeRange.end);
            }
        ...
        getIncidentTimeline: vitest_1.vi.fn((entityId) => {
            const entityIncidents = securityIncidents.filter((i) => i.entityId === entityId);
            return Promise.resolve({
                incidents: entityIncidents,
                count: entityIncidents.length,
                timeline: entityIncidents.map((i) => ({
                    id: i.id,
        ...
        generateSecurityRecommendations: vitest_1.vi.fn((incident) => {
            return Promise.resolve({
        ...
        auditUserAccess: vitest_1.vi.fn((userId) => {
            return Promise.resolve({
                userId,
                accessLogs: [
                    {
    ...
    return {
        SecurityAuditService: vitest_1.vi.fn().mockImplementation(() => ({
...
vitest_1.vi.mock('../services/parental-consent.service', () => {
    const mockParentalConsentService = {
        submitConsentRequest: vitest_1.vi.fn(() => Promise.resolve({
            id: 1,
            token: 'test-token-123',
            status: 'pending'
        })),
        verifyConsent: vitest_1.vi.fn(() => Promise.resolve({
            id: 1,
            status: 'verified',
            message: 'First consent verified successfully'
        })),
        revokeConsent: vitest_1.vi.fn(() => Promise.resolve({
            id: 1,
            status: 'revoked'
        })),
        getConsentStatus: vitest_1.vi.fn(() => Promise.resolve({
            id: 1,
            status: 'active'
        }))
    };
    return {
        ParentalConsentService: vitest_1.vi.fn().mockImplementation(() => mockParentalConsentService),
        parentalConsentService: mockParentalConsentService,
        default: mockParentalConsentService
    };
});
// Mock GDPR Rights Service
vitest_1.vi.mock('../services/gdpr-rights.service', () => {
    const mockGDPRRightsService = {
        submitAccessRequest: vitest_1.vi.fn((requestData) => {
        ...
        verifyAccessRequest: vitest_1.vi.fn(() => Promise.resolve({
            verified: true,
            requestId: 'mock-request-123',
            status: 'approved'
        })),
        getRequestStatus: vitest_1.vi.fn(() => Promise.resolve({
            requestId: 'mock-request-123',
            status: 'approved',
            submittedAt: new Date(),
            deadline: new Date(Date.now() + 30 * 24 * 60 * 60 * 1000)
        })),
        exportStudentData: vitest_1.vi.fn(() => Promise.resolve({
            success: true,
            data: {
        ...
        deleteStudentData: vitest_1.vi.fn(() => Promise.resolve({
            success: true,
            deletedAt: new Date(),
            anonymized: true
        }))
    ...
    return {
        GDPRRightsService: vitest_1.vi.fn().mockImplementation(() => mockGDPRRightsService),
        gdprRightsService: mockGDPRRightsService,
        default: mockGDPRRightsService
    };
});
// Mock Image Processing Service
const mockImageProcessingService = vitest_1.vi.hoisted(() => ({
    processImage: vitest_1.vi.fn(() => Promise.resolve({
        success: true,
        processedPath: '/processed/image.jpg'
    })),
...
vitest_1.vi.mock('../services/image-processing.service', () => ({
    ImageProcessingService: vitest_1.vi.fn().mockImplementation(() => mockImageProcessingService),
...
vitest_1.vi.mock('../services/short-timeout.service', () => {
    ...
    return {
        ShortTimeoutService: vitest_1.vi.fn().mockImplementation(() => mockShortTimeoutService),
...
vitest_1.vi.mock('../services/storage.service', () => {
    const mockStorageService = {
        ...
        findFileByChecksum: vitest_1.vi.fn((_checksum) => Promise.resolve(null)), // No duplicates in test
        getStorageStats: vitest_1.vi.fn(() => Promise.resolve({
            totalFiles: 10,
            totalSize: 1024000,
            availableSpace: 9000000000,
        ...
        cleanupExpiredFiles: vitest_1.vi.fn((_daysOld) => Promise.resolve(5)), // Cleaned 5 files
        getStorageHealth: vitest_1.vi.fn(() => Promise.resolve({
            status: 'healthy',
            diskUsage: 0.1,
            availableSpace: 9000000000,
    ...
    return {
        StorageService: vitest_1.vi.fn().mockImplementation(() => mockStorageService),
...
vitest_1.vi.mock('../services/real-time-progress.service', () => ({
    realTimeProgressService: {
        sendProgressUpdate: vitest_1.vi.fn(),
    },
}));
// Mock SuperMemo Service
vitest_1.vi.mock('../services/supermemo.service', () => {
    const mockCalculateNextReview = vitest_1.vi.fn((card, quality) => {
    ...
    const mockSuperMemoService = {
        ...
        updateCard: vitest_1.vi.fn(() => Promise.resolve({
            success: true,
            newInterval: 2
        })),
        getCardStats: vitest_1.vi.fn(() => Promise.resolve({
            totalCards: 10,
            dueCards: 3,
            newCards: 2
        }))
...
vitest_1.vi.mock('../services/enhanced-database.service', () => {
    const mockEnhancedDatabaseService = {
        getStudentById: vitest_1.vi.fn((studentId) => {
            // Return mock student data for student ID 1
            if (studentId === 1) {
                return Promise.resolve({
                    id: 1,
        ...
        getStudentRecommendations: vitest_1.vi.fn((_studentId, limit = 5) => {
            return Promise.resolve([
                {
                    id: 1,
                    titre: 'Addition CP',
        ...
        submitExerciseAttempt: vitest_1.vi.fn((studentId, attemptData) => {
            return Promise.resolve({
                id: 'mock-attempt-123',
                studentId: studentId,
                exerciseId: attemptData.exerciseId,
        ...
        getStudentProgress: vitest_1.vi.fn((_studentId, _exerciseIds) => {
            return Promise.resolve([
                {
                    exerciseId: 1,
                    score: 90,
        ...
        getCompetencePrerequisites: vitest_1.vi.fn((competenceCode) => {
            return Promise.resolve([
                {
                    id: 1,
        ...
        recordStudentProgress: vitest_1.vi.fn((studentId, attemptData) => {
            return Promise.resolve({
                id: 1,
                studentId: studentId,
                exerciseId: attemptData.exerciseId,
    ...
    return {
        EnhancedDatabaseService: vitest_1.vi.fn().mockImplementation(() => mockEnhancedDatabaseService),
...
vitest_1.vi.mock('../utils/errorHandler.config', () => ({
    getErrorHandlerConfig: vitest_1.vi.fn(() => ({
        environment: 'test',
        logLevel: 'debug',
...
vitest_1.vi.mock('../utils/errorLogger', () => ({
    ErrorLogger: {
        logError: vitest_1.vi.fn()
    }
}));
vitest_1.vi.mock('../utils/errorMonitoring', () => ({
    ErrorMonitoringService: {
        captureError: vitest_1.vi.fn()
    }
}));
vitest_1.vi.mock('../utils/errorResponseFormatter', () => ({
    ErrorResponseFormatter: {
        formatResponse: vitest_1.vi.fn((error, _context, _config) => ({
            success: false,
            error: {
...
vitest_1.vi.mock('../utils/requestContextExtractor', () => ({
