...
exports.createCorsMiddleware = exports.CorsConfigurationService = void 0;
...
class CorsConfigurationService {
    constructor() {
        ...
        this.validateOrigin = (origin, callback) => {
            ...
            if (this.allowedOrigins.has(origin)) {
                logger_1.logger.debug('CORS origin allowed', { origin });
                return callback(null, true);
            }
            ...
            if (process.env.NODE_ENV === 'production') {
                ...
                if (isSubdomainAllowed) {
                    logger_1.logger.info('CORS subdomain allowed', { origin });
                    return callback(null, true);
                }
                // Log security incident
                logger_1.logger.warn('CORS origin blocked', {
                    origin,
            ...
            logger_1.logger.warn('CORS origin not in allowlist (development mode)', { origin });
            return callback(null, true);
        };
    ...
    initializeAllowedOrigins() {
        ...
        if (process.env.NODE_ENV === 'development') {
            origins.push('http://localhost:3000', 'http://localhost:3001', 'http://localhost:3004', 'http://127.0.0.1:3000', 'http://127.0.0.1:3001', 'http://127.0.0.1:3004', 'http://localhost:5173', // Vite default
            'http://localhost:4173' // Vite preview
            );
        }
        origins.forEach(origin => this.allowedOrigins.add(origin));
        logger_1.logger.info('CORS origins initialized', {
    ...
    addAllowedOrigin(origin) {
        ...
        logger_1.logger.info('CORS origin added', { origin });
    }
    /**
    ...
    removeAllowedOrigin(origin) {
        ...
        if (removed) {
            logger_1.logger.info('CORS origin removed', { origin });
        }
        return removed;
    }
    /**
...
exports.CorsConfigurationService = CorsConfigurationService;
/**
            ...
            if (request.headers.origin && request.headers.referer) {
                try {
                    ...
                    if (originUrl.hostname !== refererUrl.hostname) {
                        logger_1.logger.warn('Origin/Referer mismatch detected', {
...
const corsPlugin = async (fastify) => {
    ...
    if (process.env.NODE_ENV === 'development') {
        fastify.get('/api/debug/cors-config', async (request, reply) => {
            return {
