...
exports.pathSecurity = exports.PathSecurityService = void 0;
...
class PathSecurityService {
    constructor() {
        this.securityConfig = {
            ...
            allowedDirectories: [
                path.resolve(config_1.config.upload?.path || './uploads'),
                path.resolve('./public'),
                path.resolve('./temp'),
                path.resolve('./cache')
            ],
     ...
     */
    validatePath(inputPath, baseDirectory) {
        const errors = [];
        ...
        if (normalizedPath !== inputPath) {
            logger_1.logger.warn('Path was modified during normalization', {
                original: inputPath,
        ...
        if (securityViolations.length > 0) {
            logger_1.logger.error('Path security violations detected', {
                inputPath,
                normalizedPath,
                resolvedPath,
     ...
     */
    async safeFileAccess(filePath, operation = 'read') {
     ...
     */
    createSecureFilename(originalFilename, preserveExtension = true) {
        if (!originalFilename) {
     ...
     */
    performAdvancedSecurityChecks(resolvedPath, errors, securityViolations) {
    ...
    updateSecurityConfig(updates) {
        ...
        logger_1.logger.info('Path security configuration updated', { updates });
    }
    /**
...
exports.pathSecurity = PathSecurityService.getInstance();
