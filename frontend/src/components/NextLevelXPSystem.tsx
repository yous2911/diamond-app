...
const NextLevelXPSystem: React.FC<NextLevelXPSystemProps> = React.memo(({
  currentXP,
  xp,
  maxXP,
  level,
  ...
  const actualXP = currentXP ?? 0;
  const lastXPRef = useRef(actualXP);
  ...
  // Size configurations - Memoized for performance
  ...
  const progress = Math.min((actualXP / maxXP) * 100, 100);
  ...
  const renderLiquidXP = useCallback((ctx: CanvasRenderingContext2D) => {
    ...
    if (fillWidth > 0) {
      ...
      // Level up particle eactualXPlosion
      if (isLevelingUp) {
    ...
    ctx.fillStyle = 'rgba(0, 0, 0, 0.5)';
          ctx.fillText(`${actualXP} / ${maxXP} XP`, width / 2 + 1, height / 2 + 1);
    
    // Main text
    ctx.fillStyle = 'white';
    ctx.strokeStyle = 'rgba(0, 0, 0, 0.8)';
    ctx.lineWidth = 1;
          ctx.strokeText(`${actualXP} / ${maxXP} XP`, width / 2, height / 2);
      ctx.fillText(`${actualXP} / ${maxXP} XP`, width / 2, height / 2);
    
    ctx.restore();
      }, [progress, sizeConfig, themeConfig, actualXP, maxXP, isLevelingUp]);
  ...
  useEffect(() => {
    // Calculate XP gained
    const actualXPGained = actualXP - lastXPRef.current;
    
    // Detect XP gain
    if (actualXP > lastXPRef.current && actualXPGained > 0) {
      setShowXPGain(true);
      
      // Create floating number
      const newFloating = {
        id: Date.now(),
        value: actualXPGained,
      ...
      for (let i = 0; i < Math.min(actualXPGained / 10, 20); i++) {
    ...
    if (level > lastLevelRef.current) {
      ...
      // Massive particle eactualXPlosion
      for (let i = 0; i < 50; i++) {
        liquidDropsRef.current.push(
          createLiquidDrop(
    ...
    lastXPRef.current = actualXP;
    lastLevelRef.current = level;
  }, [actualXP, level, streakActive, bonusMultiplier, progress, maxXP, themeConfig, sizeConfig, createLiquidDrop, onLevelUp, onMilestone]);
  ...
  return (
    <div className="relative inline-block">
      ...
      <div className="mt-2 text-xs text-gray-500 text-center">
        {maxXP - actualXP > 0 ? (
          `${maxXP - actualXP} XP jusqu'au niveau ${level + 1}`
        ) : (
