...
const AdvancedMascotSystem: React.FC<AdvancedMascotProps> = React.memo(({
  mascotType,
  studentData,
  currentActivity,
  equippedItems,
  ...
  // Memoized mascot type configuration for performance
  const mascotConfig = useMemo(() => {
    const configs = {
      dragon: { scale: 1.2, color: '#FF6B35', personality: 'bold' },
      fairy: { scale: 0.8, color: '#FF69B4', personality: 'gentle' },
      robot: { scale: 1.0, color: '#4ECDC4', personality: 'logical' },
      cat: { scale: 0.9, color: '#45B7D1', personality: 'curious' },
      owl: { scale: 1.1, color: '#96CEB4', personality: 'wise' }
    };
    return configs[mascotType] || configs.dragon;
  }, [mascotType]);

  // Advanced AI Decision Making - Memoized for performance
  ...
  useEffect(() => {
    ...
    // Skip 3D rendering in test environment
